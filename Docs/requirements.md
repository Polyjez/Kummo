...
These points condition the specifications and must be settled before iteration 1.

| Code | Decision | Impact |
|---|---|---|
| **DEC-01** | Does the payment for the final service between client and vendor **transit through the platform**, or does the platform only bill **the connection**? | Determines the extent of transactional needs and part of the architecture. |
| **DEC-02** | After connection, do the parties exchange via **communicated contact details** or via an **internal messaging** system? | Impacts FR-73, privacy, and the v1 scope. |
| **DEC-03** | Which **monetization model** (§4): pay-per-lead, contact unlock, subscription, success commission, or a combination? | Conditions all of section 5.6. |
| **DEC-04** | Is the **dynamic content** (vendor descriptions, categories) **multilingual** (DE + EN) or German only? | A **data schema** decision to make early (costly to retrofit). |
| **DEC-05** | How does a vendor define their **service area**: radius around a point, administrative zones (cantons/municipalities/postal codes), or a combination? | Determines the geospatial model and the entry ergonomics. |
| **DEC-06** | Which **regulatory scope**: Switzerland only (nFADP), or also EU users (GDPR)? | Impacts the compliance requirements (§6.6). |
| **DEC-07** | What are the **initial criteria and weights** of the matchmaking engine, beyond category / distance / availability? | Refines FR-31. |
